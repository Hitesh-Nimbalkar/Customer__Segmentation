...
class IngestedDataValidation:
    ...
    def check_column_names(self) -> bool:
        try:
            ...
            if collections.Counter(df_column_names) == collections.Counter(schema_column_names):
                logging.info("Column names match the schema. Validated column names:")
                self.log_column_names(df_column_names)
                return True
            else:
                logging.info("Column names do not match the schema. Not validated column names:")
                self.log_column_names(df_column_names)
                return False

        except Exception as e:
            logging.info(f"An exception occurred: {e}", exc_info=True)
            raise ApplicationException(e, sys) from e

    def log_column_names(self, column_names):
